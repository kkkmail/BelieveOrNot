...
{
    public async Task<KingGameStateDto> CreateOrJoinKingMatch(KingCreateMatchRequest request)
    {
        Console.WriteLine($"{nameof(KingHub)}.{nameof(CreateOrJoinKingMatch)} - playerName: {request.PlayerName}, Context.ConnectionId: {Context.ConnectionId}");
        ...
        try
        {
            ...
            // {
            //     Console.WriteLine($"{nameof(KingHub)}.{nameof(CreateOrJoinKingMatch)} - match.Id: {match.Id}, connection.Key: {connection.Key}, connection.Value: (MatchId: {connection.Value.MatchId}, PlayerId: {connection.Value.PlayerId})");
            // }

            var state = _gameEngine.CreateGameStateDto(match);
            await Clients.Group($"kingmatch:{match.Id}").SendAsync("StateUpdate", state);

            // Console.WriteLine($"{nameof(KingHub)}.{nameof(CreateOrJoinKingMatch)} - Successfully created match: {match} and sent StateUpdate, state: {state}");
            return state;
        }
        catch(Exception ex)
        {
            Console.WriteLine($"{nameof(KingHub)}.{nameof(CreateOrJoinKingMatch)} - Exception: {ex.Message}");
